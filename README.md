...
- **多供应商配置档案**：DeepSeek / 硅基流动 / 百炼 / 智谱 / Kimi / OpenRouter / OpenAI 每家独立保存地址、Key、模型与思考开关，一键或快捷键（`Alt+O`）切换；支持从服务商自动拉取模型列表
- **手机显示模式**：电脑端在局域网启动服务，手机扫码即可实时查看解答；截屏、追加截图、切换场景、深度思考均可在手机端操作，答题时电脑端零键盘输入
- **实时面试助手**：持续监听面试官提问（语音转写实时可见），自动识别"问完了"并实时生成建议回答，双端时间线展示；历史自动压缩
- **深度思考**：难题时开启，模型先推理再作答（随供应商配置档案保存）
- **点击截屏**：鼠标左键双击 / 单击触发截图（可选，避免快捷键被检测）
- **透明答案模式**：背景完全透明仅答案文字悬浮，字体颜色可调，配合鼠标穿透 + 滚轮滚动阅读
- **场景化提示词**：预置「解算法题」「英语考试」「通用问答」等场景，一键切换；也可以添加自定义场景，扩展到任意题型
- **自动更新**（Windows）：自动检查 GitHub 最新版本并后台下载，退出时自动覆盖安装到当前目录
...
> 直接下载安装包使用：前往 [Releases 页面](https://github.com/simpleqt/snanswer/releases) 下载最新版（Windows 为向导式安装、可选安装路径并支持自动更新；macOS 下载 dmg 后拖入应用程序）。
